...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500">Complete el formulario para agregar un nuevo empleado.</p>
        </div>
    ...
    <form action="{{ route('empleado.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="nombre" name="nombre" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nombre') border-red-500 @enderror" placeholder="Ej: Juan" value="{{ old('nombre') }}">
                @error('nombre')
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="apellido" name="apellido" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('apellido') border-red-500 @enderror" placeholder="Ej: Pérez" value="{{ old('apellido') }}">
                @error('apellido')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="email" id="correo" name="correo" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('correo') border-red-500 @enderror" placeholder="Ej: juan.perez@example.com" value="{{ old('correo') }}">
                @error('correo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="telefono" name="telefono" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('telefono') border-red-500 @enderror" placeholder="Ej: 123456789" value="{{ old('telefono') }}">
                @error('telefono')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="id_tipo_usuario" name="id_tipo_usuario" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_tipo_usuario') border-red-500 @enderror">
                    ...
                    @foreach($tipos_usuario as $tipo)
                        <option value="{{ $tipo->codigo }}" {{ old('id_tipo_usuario') == $tipo->codigo ? 'selected' : '' }}>{{ $tipo->nombre }}</option>
                    @endforeach
                </select>
