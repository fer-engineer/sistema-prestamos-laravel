...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Editar Equipo</h1>
            <p class="text-sm text-gray-500">Actualice el formulario para modificar el equipo.</p>
        </div>
    ...
    <form action="{{ route('equipo.update', $equipo->codigo) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <select id="id_marca" name="id_marca" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_marca') border-red-500 @enderror">
                    ...
                    @foreach($marcas as $marca)
                        <option value="{{ $marca->codigo }}" {{ old('id_marca', $equipo->id_marca) == $marca->codigo ? 'selected' : '' }}>{{ $marca->nombre }}</option>
                    @endforeach
                </select>
                @error('id_marca')
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="modelo" name="modelo" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('modelo') border-red-500 @enderror" placeholder="Ej: X1000" value="{{ old('modelo', $equipo->modelo) }}">
                @error('modelo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="descripcion" name="descripcion" rows="3" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('descripcion') border-red-500 @enderror" placeholder="Descripción del equipo">{{ old('descripcion', $equipo->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="id_estado" name="id_estado" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_estado') border-red-500 @enderror">
                    ...
                    @foreach($estados as $estado)
                        <option value="{{ $estado->codigo }}" {{ old('id_estado', $equipo->id_estado) == $estado->codigo ? 'selected' : '' }}>{{ $estado->nombre }}</option>
                    @endforeach
                </select>
                @error('id_estado')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="space-y-2">
                ...
                <input type="date" id="fecha_adquisicion" name="fecha_adquisicion" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('fecha_adquisicion') border-red-500 @enderror" value="{{ old('fecha_adquisicion', $equipo->fecha_adquisicion) }}">
