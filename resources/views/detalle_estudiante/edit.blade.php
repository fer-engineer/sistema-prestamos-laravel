...
@section('title', 'Editar Detalle de Estudiante')

@section('content')
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8 border-b border-gray-200">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Editar Detalle de Estudiante</h1>
            <p class="text-sm text-gray-500">Actualice el formulario para modificar el detalle del estudiante.</p>
        </div>
    ...
    <form action="{{ route('detalle_estudiante.update', $detalle->codigo) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="nie" name="nie" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nie') border-red-500 @enderror" placeholder="Ej: 12345" value="{{ old('nie', $detalle->nie) }}">
                @error('nie')
            ...
            <div class="space-y-2">
                ...
                <select id="empleado_id" name="empleado_id" class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('empleado_id') border-red-500 @enderror">
                    ...
                    @foreach($empleados as $empleado)
                        <option value="{{ $empleado->codigo }}" {{ old('empleado_id', $detalle->empleado_id) == $empleado->codigo ? 'selected' : '' }}>{{ $empleado->nombre }} {{ $empleado->apellido }}</option>
                    @endforeach
                </select>
                @error('empleado_id')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
